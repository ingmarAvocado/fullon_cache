...
from . import (
    # Cache-specific examples (recommended)
    example_base_cache, example_account_cache, example_bot_cache, 
    example_tick_cache, example_orders_cache, example_trades_cache,
    example_process_cache, example_ohlcv_cache,
    
    # Legacy multi-purpose examples  
    basic_usage, bot_coordination, performance_test, pubsub_example, queue_patterns
)
...
def get_cache_examples() -> dict:
    ...
    return {
        'example_base_cache': {
            'description': 'BaseCache operations - Redis fundamentals, connections, error queues',
            'cli': 'python example_base_cache.py --operations basic --keys 100 --verbose',
            'features': ['Redis operations', 'Connection testing', 'Error queues', 'Context managers', 'Key patterns']
        },
        'example_account_cache': {
            ...
            'features': ['Account balances', 'Position tracking', 'Portfolio analysis', 'Multi-account management']
        },
        'example_bot_cache': {
            ...
            'features': ['Multi-bot coordination', 'Exchange blocking', 'Status tracking', 'Conflict resolution']
        },
        'example_tick_cache': {
            'description': 'TickCache operations - real-time tickers, pub/sub, price monitoring',
            'cli': 'python example_tick_cache.py --operations pubsub --symbols BTC/USDT,ETH/USDT --duration 20',
            'features': ['Real-time tickers', 'Pub/sub patterns', 'Price monitoring', 'Multi-exchange data']
        },
        'example_orders_cache': {
            'description': 'OrdersCache operations - FIFO queues, status tracking, batch processing',
            'cli': 'python example_orders_cache.py --operations all --orders 100 --batch-size 20',
            'features': ['FIFO order queues', 'Status tracking', 'Batch processing', 'Order lifecycle']
        },
        'example_trades_cache': {
            ...
            'features': ['Trade queues', 'Status management', 'Analytics', 'Bulk processing', 'Statistics']
        },
        'example_process_cache': {
            'description': 'ProcessCache operations - monitoring, filtering, health tracking',
            'cli': 'python example_process_cache.py --operations monitoring --duration 30 --verbose',
            'features': ['Process monitoring', 'Time-based filtering', 'Health tracking', 'Statistics']
        },
        'example_ohlcv_cache': {
            'description': 'OHLCVCache operations - candlestick data, timeframes, analysis',
            'cli': 'python example_ohlcv_cache.py --operations all --bars 200 --timeframes 1m,5m,1h',
            'features': ['OHLCV data', 'Multiple timeframes', 'Data analysis', 'Performance testing']
        }
    }
