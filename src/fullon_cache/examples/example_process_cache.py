...
try:
    ...
    from ..process_cache import ProcessCache, ProcessType
...
async def basic_process_operations(
    cache: ProcessCache, process_count: int = 10, verbose: bool = False
) -> bool:
    ...
    try:
        ...
        for i in range(process_count):
            ...
            proc_type = ProcessType(process_type)  # Convert string to enum
            await cache.register_process(
                proc_type, process_id, {"demo": True}, f"Demo process for {process_type}"
            )
        ...
        for process_type in {pt for pt, _ in added_processes}:
            proc_type_enum = ProcessType(process_type)
            processes = await cache.get_active_processes(proc_type_enum)

            expected_count = sum(1 for pt, _ in added_processes if pt == process_type)
            if len(processes) < expected_count:
                print(
                    f"❌ Process count mismatch for {process_type}: "
                    + f"expected {expected_count}, got {len(processes)}"
                )
                return False
...
async def process_monitoring_demo(
    cache: ProcessCache, duration: int = 30, verbose: bool = False
) -> bool:
...
async def simulate_process_activity(
    cache: ProcessCache, process_type: str, process_id: str, duration: int, verbose: bool = False
) -> dict[str, int]:
...
async def monitor_system_processes(
    cache: ProcessCache, monitored_processes: list, duration: int, verbose: bool = False
) -> bool:
...
async def process_filtering_demo(cache: ProcessCache, verbose: bool = False) -> bool:
    ...
    try:
        ...
        if len(all_filter_processes) >= len(test_data):
            ...
            print(
                f"❌ Missing processes: expected {len(test_data)}, "
                + f"found {len(all_filter_processes)}"
            )
            return False
...
async def run_demo(args) -> bool:
    ...
    if args.operations in ["monitoring", "all"]:
        async with ProcessCache() as cache:
            results["monitoring"] = await process_monitoring_demo(
                cache, args.duration, args.verbose
            )
...
def main():
    ...
    parser.add_argument(
        "--verbose", "-v", action="store_true", help="Verbose output with detailed process info"
    )
