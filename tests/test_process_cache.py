...
class TestProcessCacheIntegration:
    ...
    async def test_performance_many_processes(self, process_cache, benchmark_async):
        ...
        assert stats['mean'] < 0.5  # Should complete in under 500ms (relaxed for CI)
...
class TestProcessCacheLegacyMethods:
    ...
    async def test_legacy_edge_cases(self, process_cache):
        ...
        await process_cache._cache.hset(f"active:bot", p2, datetime.now(timezone.utc).isoformat())
        ...
        await process_cache._cache.hset(f"active:bot", p3, datetime.now(timezone.utc).isoformat())
    ...
    async def test_legacy_edge_cases_continued(self, process_cache):
        ...
        # Clean up test data
        await process_cache._cache.hdel(f"active:bot", fake_process_id, fake_id, stale_id)
        ...
        old_active_time = (datetime.now(timezone.utc) - timedelta(minutes=40)).isoformat()
        await process_cache._cache.hset(
            f"active:bot",
            fresh_process_id,
            old_active_time
